...
lib = cdll.LoadLibrary('%s/librf24-bcm.so.1.0' % abs_path)
...
class RF24(object):
    ...
    def stopListening(self):
        lib.RF24_stopListening(self.obj)

    def write(self, buf, length, multicast=False):
        if multicast:
            return lib.RF24_write_2(self.obj, buf, length, True)
        else:
    ...
    def get_pipe_num(self):
        pipe_num = c_int()
        lib.RF24_available_2(self.obj, byref(pipe_num))
        return pipe_num.value

    def read(self, buf, size=None):
        # Python doesn't direclty support referencable parameters (the C++ library uses these), but since it's dynamically typed, we can return an arbitray object
        if type(buf) is bool:
           to_return  = c_bool(buf)
           if size is None:
             length = sys.getsizeof(c_bool)
        elif type(buf) is str:
            to_return = create_string_buffer(buf)
            if size is None:
              length = sys.getsizeof(c_char_p)
        elif type(buf) is unicode:
            if len(buf) == 1:
                to_return = c_wchar(buf)
                if size is None:
                  length = sys.getsizeof(c_wchar)
            else:
                to_return = c_wchar_p(buf)
                if size is None:
                  length = sys.getsizeof(c_wchar_p)
        elif type(buf) is int:
            to_return = c_long(buf)
            if size is None:
              length = sys.getsizeof(c_longlong)
        elif type(buf) is long:
            to_return = c_long(buf)
            if size is None:
              length = sys.getsizeof(c_longlong)
        elif type(buf) is float:
            to_return = c_longdouble(buf)
            if size is None:
              length = sys.getsizeof(c_longdouble)
        elif type(buf) is dict:
            raise Exception("You cannot submit a dictionary to the method RF24.read()")
        elif type(buf) is list:
            raise Exception("You cannot submit a list to the method RF24.read()")
        if size is not None:
          length = size
        lib.RF24_read(self.obj, byref(to_return), length)
        print("%s, %s" % (to_return.value, to_return))
    ...
    def openReadingPipe(self, number, address):
        lib.RF24_openReadingPipe(self.obj, number, address)
    ...
    def setPayloadSize(self, size):
        lib.RF24_setPayloadSize(self.obj, size)
    ...
    def setAutoAck(self, enable, pipe=None):
        if pipe is not None:
